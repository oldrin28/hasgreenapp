...
export function DevicesListScreen() {
  ...
  const { loadDevices, devices, isLoading } = useDevices();
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: activeColors.background }]} edges={['top']}>
        ...
        {activeTab === 'dispositivos' ? (
          ...
          <View style={styles.devicesGrid}>
            {devices.map((device: any) => {
              ...
              const isRouter = String(device.device_type || '').toLowerCase().includes('router') || 
                               String(device.device_name || '').toLowerCase().includes('casa') || 
                               String(device.device_type || '').toLowerCase().includes('gateway');
                  ...
                  <Card layer="lowest" elevation="sm" style={[styles.deviceCard, { borderLeftWidth: 4, borderLeftColor: isActive ? activeColors.primary : activeColors.outlineVariant }]}>
                    <View style={styles.cardTopRow}>
                      <View style={[styles.iconBox, { backgroundColor: activeColors.primaryContainer }]}>
                        <MaterialIcons name={isRouter ? "router" : "developer-board"} size={32} color={activeColors.primary} />
                      </View>
                    ...
                    <View style={styles.cardMidRow}>
                      ...
                      <Typography variant="body" color="onSurfaceVariant" style={{ fontWeight: '500', fontSize: 14 }}>{device.device_type || 'Dispositivo IoT'}</Typography>
                    </View>
            ...
            {/* Empty State / Add New */}
            <Pressable onPress={() => router.push('/nuevo-dispositivo' as any)}>
          ...
          /* Alarms Grid Placeholders */
          <View style={{ gap: Spacing[6], marginBottom: Spacing[6] }}>
            <Card layer="lowest" style={{ padding: Spacing[6], alignItems: 'center' }}>
              ...
              <Typography variant="headline" style={{ fontSize: 18, marginBottom: Spacing[2] }}>Historial de Alarmas</Typography>
              <Typography variant="body" color="onSurfaceVariant" style={{ textAlign: 'center' }}>
                No hay alertas críticas en el sistema. Todos los dispositivos operan con normalidad.
              </Typography>
            </Card>
          </View>
        )}
      ...
      {/* FAB */}
      <Pressable style={[styles.fab, { backgroundColor: activeColors.primary }]} onPress={() => router.push('/nuevo-dispositivo' as any)}>
